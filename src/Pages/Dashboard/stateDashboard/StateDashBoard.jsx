...
const StateDashBoard = () => {
  ...
  const fetchTopDelaer = async () => {
    try {
      const response = await common.topDealers()     
  ...
  useEffect(() => {
    const fetchOrderData = async () => {
      try {
        const response = await order.getOrderCountByState(selectedState, "StateManager");
        if (response.data) {
  ...
  const fetchCompnayKWSales = async () => {
    try {
      ...
      setcompnayKw(response.data?.totalConformKilowatt); // Assuming the data is in response.data
    } catch (error) {
  ...
  return (
    <div className="p-6 bg-gray-900 text-gray-100 min-h-screen">
      ...
      <div className="text-2xl font-bold mb-6">HYSON Total Kilowatt: {compnayKw} kW</div>
      <div>
        <h1 className="text-xl font-bold mb-2">Top Dealers</h1>
      </div>
        ...
        {(topSalesData || []).map((profile, index) => (
          ...
          >
            <img
              ...
              alt={profile?.dealerInfo?.name}
              className="w-16 h-16 rounded-full mb-2"
            />
            <div className="text-center">
              <h4 className="font-bold">{profile?.dealerInfo?.name}</h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        {barChartData !== "" ? <div className="bg-gray-800 rounded-lg p-4 relative">
          <Bar
            data={barChartData}
          />
