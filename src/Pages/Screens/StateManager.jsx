...
const StateManager = () => {
    ...
    const fetchStateManager = async () => {
      ...
      try {
        ...
        if (searchTerm || selectedState) {
          response = await common.searchstateManager(currentPage, queryParams);
          const { results, total } = response.data;

          setStateManager(results); // For search, use 'data'
          setTotalPages(Math.ceil(total / 10)); // Assuming limit is 10 per page
        } else {
          response = await common.getstateManager(currentPage);
  ...
  const fetchOrders = async (page) => {
    ...
    try {
      ...
      setorderTotalPages(response.data.total);
    } catch (error) {
      ...
      <div>
        ...
        <div className="bg-white dark:bg-boxdark rounded-lg shadow-md overflow-hidden mb-10">
          <div className="p-3 border-b border-gray-200 dark:border-gray-700 flex align-middle justify-between">
            <h2 className="text-xl font-bold ">State Managers</h2>
            <Link to="/addstatemanger">
              <button className="flex items-center justify-center gap-1 text-indigo-500 hover:text-indigo-700">
                <CiSquarePlus />
                <span>Add StateManager</span>
              </button>
            </Link>
          </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {stateManager.map((manager, index) => (
                  <tr
                    key={index}
                    className={`${index !== stateManager.length - 1
                  ...
                  >
                    ...
                    <td className="p-3 hidden lg:table-cell max-w-xs truncate">
                      {manager.address}
                    </td>
                    <td className="p-3">{manager.state}</td>
                    <td className="p-3">{manager.country}</td>
