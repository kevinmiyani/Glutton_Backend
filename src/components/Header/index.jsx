  ...
  const fetchUserData = async () => {
    ...
    if (token) {
      try {
        ...
        if (response.data) {
          return response.data // Combine decoded token with API data if needed
        } else {
          ...
          <ul className="flex items-center gap-2 2xsm:gap-4">
            {( userData.role === 'ACCOUNTS' ||
              userData.role === 'SALESMANAGER' ||
               userData.role === 'MAINTAINANECEMANAGER' || userData.role === 'LICENSEMANAGER' || userData.role === 'PROJECTHANDLER' || userData.role === 'STOREMANAGER' || userData.role === 'DESIGNER' || userData.role === "STATEMANAGER") ? ("") : (<Link to="/addexecutive">
              <button className="flex items-center justify-center gap-1 text-black dark:text-white">
                <CiSquarePlus />
                <span>Add Executive</span>
              </button>
            </Link>)}
            {!(userData.role === 'SALESMANAGER' || userData.role === "STATEMANAGER" || userData.role === "ADMIN") ? ("") : (<Link to="/createorder">
              <button className="flex items-center justify-center gap-1 text-black dark:text-white">
                <CiSquarePlus />
                <span>Add Order</span>
              </button>
            ...
            {userrole === "STATEMANAGER" || userrole === "STOCKMANAGER" || userrole === "DESIGNER" || userrole === "ACCOUNTS" ? "" : <DropdownMessage />}
            {/* Chat Notification Area */}
          </ul>
