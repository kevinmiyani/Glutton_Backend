...
const DropdownMessage = () => {
  ...
  useEffect(() => {
    ...
    socketServices.on("lastUserMessageUpdate", () => {
  ...
  const fetchCustomerChatRooms = async () => {
    ...
    try {
      ...
      console.log("Error fetching chat rooms");
    } 
  };
  ...
  return (
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      ...
      >
        <span
          className={`absolute -top-0.5 -right-0.5 z-1 h-2 w-2 rounded-full bg-meta-1 ${!notifying ? 'hidden' : 'inline'}`}
        >
